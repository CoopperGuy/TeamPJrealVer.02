#include "stdafx.h"
#include "..\Public\WolfBoss.h"
#include "StateMachine.h"
#include "BasicCollider.h"
#include "Transform.h"
#include "MonHp.h"
BEGIN(Client)
void CreateWolfThread(CWolfBoss* obj, _float3 pos) {
...
CWolfBoss::CWolfBoss(CGameObject* pObj)
	: CEnemy(pObj)
{
}


CWolfBoss * CWolfBoss::Create(CGameObject * pObj, _float3 position)
{
	CWolfBoss*		pInstance = new CWolfBoss(pObj);

	if (FAILED(pInstance->Initialize(position)))
	{
		MSG_BOX("Failed to Create CWolfBoss");
...
HRESULT CWolfBoss::Initialize(_float3 position)
{
...
void CWolfBoss::Update(_double dDeltaTime)
{
	if (!m_pGameObject)
		return;
...
void CWolfBoss::LateUpdate(_double dDeltaTime)
{
...
void CWolfBoss::Render()
{
}

void CWolfBoss::RotateBody(_double deltaTime)
{
}

void CWolfBoss::SetGameObject(CGameObject* obj)
{
	m_pGameObject = obj;
}

void CWolfBoss::SetTransform(CTransform * trans)
{
	m_pTransform = trans;
}

void CWolfBoss::SetCollision(CCollider * colli)
{
	m_pCollider = colli;
}

void CWolfBoss::SetNavigation(CNavigation * nav)
{
	m_pNavigation = nav;
}

void CWolfBoss::SetModel(CModel * model)
{
	m_pModel = model;
}

void CWolfBoss::SettingCollider(CCollider * col)
{
	if (col)
	{
...
void CWolfBoss::SetBasicCollider(CBasicCollider * BasicCol)
{
	//if (BasicCol)
	//{
...
void CWolfBoss::SetStat(CStat * pstat)
{
	m_pStat = pstat;
}

void CWolfBoss::SetAttTarget(CGameObject * obj)
{
	m_pTargetTransform = static_cast<CTransform*>(obj->GetComponent("Com_Transform"));
}

void CWolfBoss::SetAttack(_double dDeltaTime)
{
	if (!WolfAtt)
		return;
...
void CWolfBoss::SetMonHp(CMonHp * hp)
{
	m_pMonHp = hp;
}

void CWolfBoss::StateUpdate(_double dDeltaTime)
{
...
void CWolfBoss::BehaviorUpdate(_double dDeltaTime)
{
	if (m_eState == Run)
	{
		_vector vPosition = m_pTransform->GetState(CTransform::STATE_POSITION);
...
void CWolfBoss::SetAtt()
{
	WolfAtt = true;
	WolfIdle = false;
}

void CWolfBoss::SetIdle()
{
	WolfAtt = false;
	WolfIdle = true;
}
