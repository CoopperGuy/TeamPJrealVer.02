...
_uint CCamera_Follow::Update(_double TimeDelta)
{
	...
	if (GetActiveWindow() == g_hWnd && m_bRolling)
	{
		...
		if (m_pEngine->IsKeyPressed('Q'))
			targetAngle -= XMConvertToRadians(45.f) * (_float)TimeDelta * speed;
		else if (m_pEngine->IsKeyPressed('E'))		
			targetAngle += XMConvertToRadians(45.f) * (_float)TimeDelta * speed;

		_float	fFollowSpeed = (_float)TimeDelta * 6.f;
	...
	static _float fLerpSpeed = (_float)TimeDelta * 50.f;
	...
	static _float fGrenadeLerpSpeed = (_float)TimeDelta/* * 2.f*/;
...
void CCamera_Follow::Shake_Grenade()
{
	...
	while (timeElapsed < 2.f)
	{
		...
		timeElapsed += (_float)deltaTime;
		fInterval += (_float)deltaTime;
		fIntensity -= (_float)deltaTime;
	}
