...
{
	CGameObject* pTrail = CEngine::GetInstance()->AddGameObject(CEngine::GetInstance()->GetCurSceneNumber(), "Prototype_EmptyEffect", "Flogas_Trail");
	if (pTrail == nullptr)
		return;
	...
	TRAILDESC TrailDesc;
	TrailDesc.vHighOffset = { 0.f, 0.45f, 0.f };
	TrailDesc.vLowOffset = { 0.f, -0.2f, 0.f };
...
void CFlogas::OrganizeEffect(Flogas eState, _double dDeltaTime)
{
	...
	case FLYING_START: {
		...
		if (keyFrame >= 31.f)
			CEngine::GetInstance()->PlaySoundW("FlogasFlyEff.mp3", CHANNELID::ENEMY15);
		break;
	}
	case FLYING_ING:
...
void CFlogas::Hit(_double dDeltaTime)
{
	...
	m_bMakeBlood += dDeltaTime;
	if (m_pOBB->Get_isHit()) {

		CEngine::GetInstance()->PlaySoundW("Blood.mp3", CHANNELID::ENEMY18);
		//if (m_bMakeBlood >= 0.02f) {
		//	CEngine::GetInstance()->PlaySoundW("Blood.mp3", CHANNELID::ENEMY18);
		//	m_bMakeBlood = 0.0;
		//}
		//else
		//	CEngine::GetInstance()->StopSound(CHANNELID::ENEMY18);
