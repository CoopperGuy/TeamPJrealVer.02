...
_uint CSky::LateUpdate(_double TimeDelta)
{
	...
	m_pTransformCom->SetState(CTransform::STATE_POSITION, CEngine::GetInstance()->GetCamPosition());
...
HRESULT CSky::Render(_uint iPassIndex)
{
	...
	m_pVIBufferCom->GetShader()->SetUp_ValueOnShader("g_ViewMatrix", &XMMatrixTranspose(CEngine::GetInstance()->GetTransform(CPipeline::D3DTS_VIEW)), sizeof(_matrix));
	m_pVIBufferCom->GetShader()->SetUp_ValueOnShader("g_ProjMatrix", &XMMatrixTranspose(CEngine::GetInstance()->GetTransform(CPipeline::D3DTS_PROJ)), sizeof(_matrix));
...
HRESULT CSky::SetUp_Components()
{
	...
	if (FAILED(__super::SetUpComponents(0, "Prototype_VIBuffer_Cube", "Com_VIBuffer", (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
	if (FAILED(__super::SetUpComponents(0, "Prototype_SkyBox", "Com_Texture", (CComponent**)&m_pTextureCom)))
		return E_FAIL;
