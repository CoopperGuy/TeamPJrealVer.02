...
void CGrid::SetUpGrid()
{
	...
	PxBoxGeometry overlapBox = PxBoxGeometry(fHalfExtent, 0.1f, fHalfExtent);  // [in] shape to test for overlaps

	for (int i = 0; i < (int)m_Desc.iSizeX; ++i)
	{
		for (int j = 0; j < (int)m_Desc.iSizeZ; ++j)
		{
...
list<CNode*> CGrid::GetNeighbours(CNode * node)
{
	...
	for (_int x = -1; x <= 1; ++x)
	{
		for (_int z = -1; z <= 1; ++z)
		{
			...
			if (checkX >= 0 && checkX < (int)m_Desc.iSizeX &&
				checkZ >= 0 && checkZ < (int)m_Desc.iSizeZ)
			{
...
{
	if (checkX >= 0 && checkX < (int)m_Desc.iSizeX && checkZ >= 0 &&checkZ < (int)m_Desc.iSizeZ &&
