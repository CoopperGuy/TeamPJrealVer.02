...
HRESULT CWinds::Initialize(CEmptyEffect * _pThisEffect, CGameObject * _pPlayer)
{
	...
	_vector position = XMVector3TransformCoord(XMVectorSet(0.f, 0.25f, 0.f, 1.f), pPlayerTrans->GetWorldMatrix());
	m_pEffectTrans->SetState(CTransform::STATE_POSITION, position);

	return S_OK;
}

void CWinds::Update(_double deltaTime)
{
}

void CWinds::LateUpdate(_double deltaTime)
{
...
CWinds * CWinds::Create(CEmptyEffect * _pThisEffect, CGameObject * _pPlayerEffect)
{
	CWinds*		pInstance = new CWinds();

	if (FAILED(pInstance->Initialize(_pThisEffect, _pPlayerEffect)))
	{
		MSG_BOX("Failed to Create CWinds");
