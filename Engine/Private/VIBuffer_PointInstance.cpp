...
#pragma region VERTEXBUFFER
	...
	for (_uint i = 0; i < m_iNumVertices; ++i)
	{
		...
		((VTXPOINT*)m_pVertices)[i].vSize = _float2(0.3f, 0.3f);
	}
...
HRESULT CVIBuffer_PointInstance::Initialize(void * pArg)
{
	...
	{
		((VTXPOINT*)m_pVertices)[i].vPosition = _float3(0.0f, 0.0f, 0.f);
		((VTXPOINT*)m_pVertices)[i].vSize = _float2(0.5f, 0.5f);
	}
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		pIV->vPosition = _float4((rand() % 3 - 1.f) * 0.1f, -0.2f + (rand() % 5) * 0.1f, 0.f, 1.f);
		pIV->iStartFrame = 0;
...
HRESULT CVIBuffer_PointInstance::Update(_double TimeDelta)
{
	...
	sort(m_InstanceMatrices.begin(), m_InstanceMatrices.end(), [&](VTXMATRIX* SourMatrix, VTXMATRIX* DestMatrix) {
		_vector		SourPosition = XMVector4Transform(XMLoadFloat4(&SourMatrix->vPosition), CEngine::GetInstance()->GetViewMatrix());
		_vector		DestPosition = XMVector4Transform(XMLoadFloat4(&DestMatrix->vPosition), CEngine::GetInstance()->GetViewMatrix());
...
HRESULT CVIBuffer_PointInstance::Set_Position(_float3 vPosition)
{
	D3D11_MAPPED_SUBRESOURCE		SubResource;
	...
	((VTXPOINT*)SubResource.pData)->vPosition = vPosition;
