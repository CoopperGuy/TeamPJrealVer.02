...
CModel::CModel(const CModel & rhs)
	...
	, m_pShaderFilePath(rhs.m_pShaderFilePath)
	, m_pEffectFilePath(rhs.m_pEffectFilePath)
	...
	, m_bIsAnimationMesh(rhs.m_bIsAnimationMesh)
{
	m_pxVertices = new PxVec3[m_iNumVertices];
	memcpy(m_pxVertices, rhs.m_pxVertices, sizeof(PxVec3) * m_iNumVertices);
	...
	for (auto& texture : rhs.m_ModelTextures)
	{
		_uint iNum = 0;
		...
		for (auto& tex : texture->pModelTexture)
		{
			if (tex)
			{
				pModelTexture->pModelTexture[iNum] = tex;
				SafeAddRef(tex);
			}
			++iNum;
		}
...
_fmatrix CModel::Get_BoneWithoutOffset(const char * pBoneName)
{
	CHierarchyNode*		pNode = Find_HierarchyNode(pBoneName);
	if (nullptr == pNode)
		return XMMatrixIdentity();

	return pNode->Get_CombinedTransformationMatrix();
}

_fmatrix CModel::Get_TransformationMatrix(const char * pBoneName)
{
	CHierarchyNode*		pNode = Find_HierarchyNode(pBoneName);
	if (nullptr == pNode)
		return XMMatrixIdentity();

	return XMLoadFloat4x4(&pNode->Get_TransformationMatrix());
}

HRESULT CModel::CreateBuffer(string pMeshFilePath, string pMeshFileName, string pShaderFilePath, string pEffectFilePath, _fmatrix _piviot)
{
	if (pMeshFileName.length() == 0 || pMeshFileName.length() == 0)
		return S_OK;

	XMStoreFloat4x4(&m_matPivot, _piviot);
	...
	m_pEffectFilePath = pEffectFilePath;

	string strFullPath = pMeshFilePath + pMeshFileName;

	m_pScene = m_Importer.ReadFile(strFullPath, aiProcess_ConvertToLeftHanded | aiProcess_CalcTangentSpace | aiProcess_Triangulate);
	...
	for (_uint i = 0; i < m_pScene->mNumMeshes; ++i)
	{
		if (FAILED(Create_MeshContainer(m_pScene->mMeshes[i], &iStartVertexIndex, &iStartFaceIndex,XMLoadFloat4x4(&m_matPivot))))
			return E_FAIL;
	}
...
HRESULT CModel::Bind_Buffers(_uint iPassIndex, CTransform * pTarget)
{
	...
	if (m_pShaderFilePath == "../../Assets/Shader/Shader_Mesh.fx")
	{
		if (m_bLinkEquip)
		{
			if (pTarget)
				m_pShader->SetUp_ValueOnShader("g_WorldMatrix", &XMMatrixTranspose(XMLoadFloat4x4(&pTarget->GetMatrix())), sizeof(_matrix));
			else
				m_pShader->SetUp_ValueOnShader("g_WorldMatrix", &XMMatrixTranspose(XMMatrixIdentity()), sizeof(_matrix));
		}
		else {
			CTransform*	renderTrans = nullptr;
			...
			if (CEngine::GetInstance()->GetCurrentUsage() == CEngine::USAGE::USAGE_TOOL)
			{
				...
				renderTrans = static_cast<CTransform*>(m_pMaster->GetComponent("Com_RenderTransform"));
				if (m_pMaster->GetComponent("Com_Collider")) {
					m_pShader->SetUp_ValueOnShader("g_WorldMatrix", &XMMatrixTranspose(XMLoadFloat4x4(&renderTrans->GetMatrix())), sizeof(_matrix));
				}
				else {
					m_pShader->SetUp_ValueOnShader("g_WorldMatrix", &XMMatrixTranspose(XMLoadFloat4x4(&m_pTransform->GetMatrix())), sizeof(_matrix));
				}
		...
		if (iPassIndex != 4)
		{
			...
			else
			{
				...
				if (iNumLight > 0)
				{
					...
					m_pShader->SetUp_ValueOnShader("lightDir", &CLightManager::GetInstance()->GetDirection(0), sizeof(_float3));
				}
...
HRESULT CModel::Render(_uint iMaterialIndex, _uint iPassIndex)
{
	...
	if (m_pShaderFilePath == "../../Assets/Shader/Shader_Mesh.fx")
	{
		...
		if (iPassIndex == 0)
		{
			if (0 < m_Animations.size() || m_bLinkEquip)
			{
				if (m_bSimulateRagdoll)
					iPassIndex = 2;
				else
			...
			if (iNumLight > 0)
			{
				ID3D11ShaderResourceView*	pShadowSRV = CLightManager::GetInstance()->GetShaderResourceView(0);
				if (pShadowSRV)
					m_pShader->SetUp_TextureOnShader("ShadowDepthTexture", pShadowSRV);
			}
		}
	}
	...
	if (m_SortByMaterialMesh.size() > 0)
	{
...
HRESULT CModel::Create_MeshContainer(aiMesh * pMesh, _uint* pStartVertexIndex, _uint* pStartFaceIndex, _fmatrix PivotMatrix)
{
	...
	for (_uint i = 0; i < pMesh->mNumVertices; ++i)
	{
		...
		// PhysX
		memcpy(&m_pxVertices[*pStartVertexIndex], &pxPos, sizeof(PxVec3));
...
HRESULT CModel::Create_Materials(aiMaterial* pMaterial, string pMeshFilePath)
{
	...
	if (m_pEffectFilePath != "")
	{
		char	szTextureFileName[MAX_PATH] = "";
		char	szExt[MAX_PATH] = "";

		_splitpath(m_pEffectFilePath.c_str(), nullptr, nullptr, szTextureFileName, szExt);
		strcat_s(szTextureFileName, szExt);

		string EffectFilePath = "../../Assets/Textures/Effect/Diffuse/";
		string strFullPath = EffectFilePath + szTextureFileName;

		if (!strcmp(szExt, ".dds") || !strcmp(szExt, ".DDS"))
			pModelTexture->pModelTexture[aiTextureType_DIFFUSE] = CTexture::Create(m_pDevice, m_pDeviceContext, CTexture::TYPE_DDS, strFullPath);
		else if (!strcmp(szExt, ".tga") || !strcmp(szExt, ".TGA") || !strcmp(szExt, ".Tga"))
			pModelTexture->pModelTexture[aiTextureType_DIFFUSE] = CTexture::Create(m_pDevice, m_pDeviceContext, CTexture::TYPE_TGA, strFullPath);
		else
			pModelTexture->pModelTexture[aiTextureType_DIFFUSE] = CTexture::Create(m_pDevice, m_pDeviceContext, CTexture::TYPE_WIC, strFullPath);
...
HRESULT CModel::SetUp_SkinnedInfoToEquipment(CModel * pTarget)
{
	for (_uint i = 0; i < m_pScene->mNumMeshes; ++i)
	{
		...
		for (_uint j = 0; j < pMesh->mNumBones; ++j)
		{
			...
			ZeroMemory(pBoneDesc, sizeof(BONEDESC));
			pBoneDesc->pName = pBone->mName.data;
			pBoneDesc->pHierarchyNode = Find_HierarchyNode(pBone->mName.data, pTarget);
	...
	for (auto& pMeshContainer : m_MeshContainers)
	{
		...
		for (auto& pBoneDesc : Bones)
		{
			pBoneDesc->pHierarchyNode = Find_HierarchyNode(pBoneDesc->pName, pTarget);
			pBoneDesc->pHierarchyNode->Set_OffSetMatrix(XMLoadFloat4x4(&pBoneDesc->OffsetMatrix));
			SetRagdollBoneDesc(pBoneDesc);
		}
	}
...
CHierarchyNode * CModel::Find_HierarchyNode(const char * pBoneName, CModel * pTarget)
{
	auto	iter = find_if(pTarget->m_HierarchyNodes.begin(), pTarget->m_HierarchyNodes.end(), [&](CHierarchyNode* pNode)
	{
		return !strcmp(pNode->Get_Name(), pBoneName);
	});
...
HRESULT CModel::Blend_Animation(_double TimeDelta)
{
	...
	//	m_fBlendTime_Upper += (_float)TimeDelta;
...
void CModel::SetAnimationLoop(_uint idx, _bool Loop, _bool Root)
{
	m_Animations[idx]->SetLoop(Loop);
	m_Animations[idx]->SetRoot(Root);
}

_bool CModel::Get_Loop()
{
	return m_Animations[m_iAnimationIndex]->GetLoop();
}

_bool CModel::Get_Root(_bool bCur)
{
	if (m_Animations.size() > 0) {
		if (bCur)
			return m_Animations[m_iAnimationIndex]->GetRoot();
		else
			return m_Animations[m_iPrevAnimationIndex]->GetRoot();
	}
	return false;
}

_uint CModel::GetCurrentKeyFrame()
{
	return m_Animations[m_iAnimationIndex]->GetCurrentKeyFrame();
}

void CModel::Set_CurrentKeyFrame(_uint iIndex, _uint CurrentKeyFrame)
{
	m_Animations[iIndex]->Set_CurrentKeyFrame(CurrentKeyFrame);
}

void CModel::Set_CurrentKeyFrame(_uint CurrentKeyFrame)
{
	m_Animations[m_iAnimationIndex]->Set_CurrentKeyFrame(CurrentKeyFrame);
}

void CModel::Set_PrevKeyFrame(_uint PrivKeyFrame)
{
	m_Animations[m_iPrevAnimationIndex]->Set_CurrentKeyFrame(PrivKeyFrame);
	//m_Animations[m_iPrevAnimationIndex]->Update_TransformationMatrices(2.f);
}

void CModel::Set_ForceLoop(_uint Index, _bool result)
{
	m_Animations[Index]->SetForceLoop(result);
}
...
void CModel::CreatePxMesh()
{
	...
	//position = m_pTransform->GetState(CTransform::STATE_POSITION);
	...
	m_pRigidActor = pEngine->GetPhysics()->createRigidStatic(transform);
	...
	filterData.word2 = CPxManager::GROUP4;
...
HRESULT CModel::SetEffectMaterial()
{
	...
	if (m_pEffectFilePath != "")
	{
		...
		string strFullPath = m_pMeshFilePath + szTextureFileName;

		if (!strcmp(szExt, ".dds") || !strcmp(szExt, ".DDS"))
			pModelTexture->pModelTexture[aiTextureType_DIFFUSE] = CTexture::Create(m_pDevice, m_pDeviceContext, CTexture::TYPE_DDS, m_pEffectFilePath);
		else if (!strcmp(szExt, ".tga") || !strcmp(szExt, ".TGA") || !strcmp(szExt, ".Tga"))
			pModelTexture->pModelTexture[aiTextureType_DIFFUSE] = CTexture::Create(m_pDevice, m_pDeviceContext, CTexture::TYPE_TGA, m_pEffectFilePath);
		else
			pModelTexture->pModelTexture[aiTextureType_DIFFUSE] = CTexture::Create(m_pDevice, m_pDeviceContext, CTexture::TYPE_WIC, m_pEffectFilePath);
...
_bool CModel::EquipmentOn(_uint iMaterialIndex)
{
	if (m_vecEquipMaterial.size() > 0)
	{
		if (m_vecEquipMaterial[iMaterialIndex] == 1)
			return true;
	}
	return false;
}

void CModel::SetUpPxVectices()
{
	for (_uint i = 0; i < m_iNumVertices; i++) {
		_vector pxPos = XMVectorSet(m_pxVertices[i].x, m_pxVertices[i].y, m_pxVertices[i].z, 1.f);
		_vector scale, rotation, position;
		XMMatrixDecompose(&scale, &rotation, &position, m_pTransform->GetWorldMatrix());
		pxPos = XMVector3TransformCoord(pxPos , XMMatrixScalingFromVector(scale) * XMMatrixRotationQuaternion(rotation));
		_float3 tempPos;
		XMStoreFloat3(&tempPos, pxPos);
		memcpy(&m_pxVertices[i], &tempPos, sizeof(_float3));
	}
}
