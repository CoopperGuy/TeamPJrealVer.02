...
_bool CAnimation::Update_TransformationMatrices(_double TimeDelta)
{
	...
	for (auto& pAnimChannel : m_Channels)
	{
		...
		{
			while (m_CurrrentTime > KeyFrames[iCurrentKeyFrame + 1]->Time)
				pAnimChannel->Set_CurrentKeyFrame(++iCurrentKeyFrame);
...
HRESULT CAnimation::Blend_Animation(CAnimation * prevAnim, _float ratio, _bool bRoot)
{
	if (ratio >= 1.f)
	{
		ratio = 1.f;
	...
	{
		if (prevAnim->GetLoop() && m_isLoop || prevAnim->GetLoop() && !m_isLoop)
		{
		}
		else
		{
			prevAnim->Update_TransformationMatrices(ratio/* * 5.f*/);
		}
	}
	_vector		vSourScale, vDestScale;
	_vector		vSourRotation, vDestRotation;
	...
	for (; curIter != m_Channels.end() && prevIter != prevAnim->m_Channels.end(); ++curIter, ++prevIter)
	{
		...
		TransformationMatrix = XMMatrixAffineTransformation(vScale, XMVectorSet(0.f, 0.f, 0.f, 1.f), vRotation, vDestPosition);
...
void CAnimation::Set_CurrentKeyFrame(_double CurrentKeyFrame)
{
	if (!m_isLoop)
		m_CurrrentTime = CurrentKeyFrame;
}
