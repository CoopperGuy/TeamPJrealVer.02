...
Texture2D		g_FireColorTexture;
Texture2D       g_FireGreyTexture;
...
VS_OUT VS_MAIN(VS_IN In)
{
  ...
  // Scale texture coordinates and add offset.
    ...
    Out.vGreyUV = tileUV;
    Out.vColorUV = In.vTexUV;
    Out.vColorUV.y = In.vTexUV.y + (In.iStartFrame * 0.05f);
...
vector	PS_MAIN(PS_IN In) : SV_TARGET
{
    ...
    vector vGrey = g_FireGreyTexture.Sample(g_DiffuseSampler, In.vGreyUV);

    vColor = g_FireColorTexture.Sample(g_DiffuseSampler, In.vColorUV);
    vColor.a = vGrey;
