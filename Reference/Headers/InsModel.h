...
class ENGINE_DLL CInsModel final : public CComponent
{
public:
...
private:
	explicit CInsModel(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CInsModel(const CInsModel& rhs);
	virtual ~CInsModel() = default;
public:
...
public:
	HRESULT CreateBuffer(string pMeshFilePath, string pMeshFileName, string pShaderFilePath = "../../Assets/Shader/Shader_Mesh.fx", _int _InstanceCount=1, _fmatrix _piviot = XMMatrixIdentity());
	void RemoveBuffer();
public:
...
public:
	static CInsModel* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
