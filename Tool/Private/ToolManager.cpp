...
static string strScene = "../../Assets/Scenes/DarkKnightDungeonkim.yaml";
...
HRESULT CToolManager::Initialize()
{
	...
	if (FAILED(m_pEngine->ReadyDevice(g_hWnd, WINCX, WINCY, hInst)))
		return E_FAIL;
...
HRESULT CToolManager::OpenScene(SCENE eScene)
{
	...
	if (FAILED(m_pEngine->SetUpCurrentScene(pScene,0)))
		return E_FAIL;
...
void CToolManager::CreateWindows()
{
	...
	pWindow = new CNavi();
	m_pEngine->AddWindow("Navigation", pWindow);
}
