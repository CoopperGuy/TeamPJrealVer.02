...
_uint CCamera_Tool::Update(_double TimeDelta)
{
	...
	if (GetActiveWindow() == g_hWnd)
	{
		...
		{
			m_pTransformCom->GoStraight(speed * camspeed);
		}

		if (m_pEngine->IsKeyPressed('S'))
		{
			m_pTransformCom->GoBackWard(speed * camspeed);
		}

		if (m_pEngine->IsKeyPressed('A'))
		{
			m_pTransformCom->GoLeft(speed * camspeed);
		}

		if (m_pEngine->IsKeyPressed('D'))
		{
			m_pTransformCom->GoRight(speed * camspeed);
		}

		if (m_pEngine->IsKeyDown('9'))
		{
			camspeed += 1.f;
		}

		if (m_pEngine->IsKeyDown('0'))
		{
			camspeed -= 1.f;
			if (camspeed <= 0)
				camspeed = 1.f;
