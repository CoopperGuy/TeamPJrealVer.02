                     ...
                     _In_ int       nCmdShow)
{
	...
	if (FAILED(CEngine::GetInstance()->AddTimers("Timer_Default")))
		return FALSE;

	if (FAILED(CEngine::GetInstance()->AddTimers("Timer_40")))
		return FALSE;
	...
	while (true)
	{
		...
		TimeAcc += CEngine::GetInstance()->ComputeDeltaTime("Timer_Default");

		if (TimeAcc > 1.0 / 60.0)
		{
			pToolManager->Update(TimeAcc);
			pToolManager->Render();
			TimeAcc = 0.0;
		}
    }
